...
var noteId = Math.ceil(Math.random() * 1000000000000000);
var jsonData;
var abc;
var id = 1;
var txt = 1;
$(document).ready(function() {
	...
	myApp.controller('MainController', ['$scope','$http', function($scope,$http) {
		$http.get('/rest/sprintBoard/show/').success(function(jsonData) {
	...
	/*$.getJSON('/rest/sprintBoard/show/', function(jsonData) {
		//jsonData = [];
		console.log(jsonData);
		var clas="colour1";
		for (var i = 0; i < jsonData.length; i++) {
			...
			notes.append("<li id=li" + id + " class='note'><div class='" + clas + "'>"
					...
					+ " placeholder='Your content here' name='textarea'>" + jsonData[i].text + "</textarea>"
					+ "<img class='hide' src='images/close.png'/>"
					+ "<label for='voteUp' class='lbl'>"+ jsonData[i].noOfLikes +"</label>"
					+ "<img class='img2' id='change' src='images/thumbsUp.png'/>"
					+ "<input type='hidden' class='noteId' name='noteId' value=" + jsonData[i].id +"/>"
					+ "</div></li>");
			id++;
	...
	$("#save").click(function() {  
		...
		$("li.note").each(function(){
			var data = {};
			var myString = $(this).find("input.noteId")[0].value;
...
function addNewNote(clas, title, content) {
	...
	notes.append("<li id=li" + id + " class='note'><div class='" + clas + "'>"
			...
			+ "<img class='img2' id='change' src='images/thumbsUp.png'/>"
			+ "<input type='hidden' class='noteId' name='noteId' value=" + noteId +"/>"
			+ "</div></li>");
